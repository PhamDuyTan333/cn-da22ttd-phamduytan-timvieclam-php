...
<div class="container" style="margin-top: 100px; margin-bottom: 50px;">
    ...
    <div class="page-header mb-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
            <div>
                <h1 class="mb-2"><i class="fas fa-briefcase me-3 text-primary"></i>Quản lý Loại công việc</h1>
                <p class="text-muted mb-0">Danh sách các loại công việc trong hệ thống</p>
            </div>
            <div class="d-flex gap-2">
                <button class="btn btn-primary" onclick="showAddForm()">
                    <i class="fas fa-plus me-2"></i>Thêm loại công việc
                </button>
    ...
    <div id="addForm" class="card border-0 shadow-sm mb-4" style="display: none;">
        <div class="card-header bg-primary text-white py-3">
            <h5 class="mb-0"><i class="fas fa-plus-circle me-2"></i>Thêm loại công việc mới</h5>
        </div>
        <div class="card-body p-4">
            <form method="POST" action="<?php echo BASE_URL; ?>danhmuc/themloaicongviec">
                <?php echo csrf_field(); ?>
                <div class="mb-3">
                    <label class="form-label fw-bold">Tên loại công việc:</label>
                    <input type="text" name="tenloai" class="form-control" required placeholder="Ví dụ: Full-time, Part-time">
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Mô tả:</label>
                    <textarea name="mota" class="form-control" rows="3" placeholder="Mô tả về loại công việc này..."></textarea>
                </div>
                <div class="d-flex gap-2">
    ...
    <div id="editForm" class="card border-0 shadow-sm mb-4" style="display: none;">
        <div class="card-header bg-warning text-dark py-3">
            <h5 class="mb-0"><i class="fas fa-edit me-2"></i>Sửa loại công việc</h5>
        </div>
        <div class="card-body p-4">
            <form method="POST" id="formSua">
                ...
                <div class="mb-3">
                    <label class="form-label fw-bold">Tên loại công việc:</label>
                    <input type="text" name="tenloai" id="editTen" class="form-control" required>
                </div>
                <div class="mb-3">
                    <label class="form-label fw-bold">Mô tả:</label>
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-light py-3">
            <h5 class="mb-0"><i class="fas fa-list me-2"></i>Danh sách loại công việc</h5>
        </div>
                ...
                <table class="table table-hover align-middle mb-0">
                    <thead class="table-light">
                        <tr>
                            ...
                            <th style="width: 200px;">Loại công việc</th>
                            <th>Mô tả</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td colspan="4" class="text-center py-5">
                                ...
                                <p class="text-muted mb-0">Chưa có loại công việc nào</p>
                            </td>
                        </tr>
                        <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="badge bg-primary" style="font-size: 0.95rem; padding: 10px 18px;">
                                    <i class="fas fa-briefcase me-2"></i><?php echo htmlspecialchars($item['tenloai']); ?>
                                </span>
                            </td>
                            <td>
                            ...
                            <td class="text-center pe-4">
                                <button class="btn btn-sm btn-warning me-1" onclick="editItem(<?php echo $item['id']; ?>, '<?php echo htmlspecialchars($item['tenloai'], ENT_QUOTES); ?>', '<?php echo htmlspecialchars($item['mota'] ?? '', ENT_QUOTES); ?>')">
...
function editItem(id, ten, mota) {
    ...
    document.getElementById('formSua').action = '<?php echo BASE_URL; ?>danhmuc/sualoaicongviec/' + id;
    showEditForm();
}

function deleteItem(id) {
    if (confirm('Bạn có chắc chắn muốn xóa loại công việc này?')) {
        document.getElementById('formXoa').action = '<?php echo BASE_URL; ?>danhmuc/xoaloaicongviec/' + id;
